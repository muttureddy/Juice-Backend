...
const SECTION_CATEGORIES = {
  juices:      ['seasonal-juices','citrus-juices','green-juices','berry-juices','energy-shots','detox-juices','special-juices'],
  salads:      ['fruit-salad','bowls'],
  sandwiches:  ['veg-sandwiches','grilled','wraps'],
  smoothies:   ['protein-smoothies','fruit-smoothies','green-smoothies'],
};
  ...
  try {
    ...
    if (!CLOUDINARY_CLOUD_NAME || !CLOUDINARY_API_KEY || !CLOUDINARY_API_SECRET) {
      return res.status(500).json({
        message: 'Cloudinary env vars not configured. Add CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET to .env',
      });
    }
    ...
    res.json({ cloudName: CLOUDINARY_CLOUD_NAME, apiKey: CLOUDINARY_API_KEY, signature, timestamp, folder });
  } catch (err) {
...
router.get('/', async (req, res) => {
  try {
    const { category, bestseller, featured, search, available, type, section } = req.query;
    const db    = getDB();
    const query = {};
    ...
    if (!category && !section) {
      const FOOD_CATS = [...SECTION_CATEGORIES.salads, ...SECTION_CATEGORIES.sandwiches];
...
router.post('/', adminAuth, async (req, res) => {
  try {
    ...
    await logAction(req.user, 'product_added', 'Product', result.insertedId,
      { name: doc.name, price: doc.price, category: doc.category }, req);
...
router.put('/:id', adminAuth, async (req, res) => {
  try {
    ...
    await logAction(req.user, 'product_updated', 'Product', req.params.id,
      { name: result.name }, req);
